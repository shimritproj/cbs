    ...
    public class Program
    {
        ...
        public void RunInstance(string fileName)
        {
            ...
            if (resultsFileExisted == false)
                runner.PrintResultsFileHeader();
            ProblemInstance instance = ProblemInstance.Import(LoadDir + "\\Instances\\" + fileName);
            runner.SolveGivenProblem(instance);
            runner.CloseResultsFile();
        }
        ...
        public void RunExperimentSet(int[] gridSizes, int[] agentListSizes, int[] obstaclesProbs, int instances)
        {
            ...
            string currentProblemFileName = LoadDir + "\\Instances\\current problem-" + ProcFileName;
                ...
                for (int obs = 0; obs < obstaclesProbs.Length; obs++)
                {
                    ...
                    for (int ag = 0; ag < agentListSizes.Length; ag++)
                    {
                        ...
                        for (int i = 0; i < instances; i++)
                        {
                            ...
                            try
                            {
                                instance = ProblemInstance.Import(LoadDir + "\\Instances\\" + instanceName);
        ...
        //protected static readonly string[] daoMapFilenames = { @"d:\Downloads\פרויקט גמר\122\eli.boyarski-mapf-d06b29ae75e8\bin\Debug\dao_maps\den502d.map", @"d:\Downloads\פרויקט גמר\122\eli.boyarski-mapf-d06b29ae75e8\bin\Debug\dao_maps\\ost003d.map", @"d:\Downloads\פרויקט גמר\122\eli.boyarski-mapf-d06b29ae75e8\bin\Debug\dao_maps\\brc202d.map" };
        ...
        public static string ProcFileName;
        ...
        {
            LoadDir = ConfigurationManager.AppSettings["LoadDir"] ?? Directory.GetCurrentDirectory();
            CsvFileName = @"CPF-experiment.csv";
            ProcFileName = @"CPF-experiment.exe";
            string csvFilePath = Path.Combine(LoadDir, CsvFileName);
            ...
            Program.RESULTS_FILE_NAME = csvFilePath;
            ...
            if (Directory.Exists(LoadDir + "\\Instances") == false)
            {
                Directory.CreateDirectory(LoadDir + "\\Instances");
            }
